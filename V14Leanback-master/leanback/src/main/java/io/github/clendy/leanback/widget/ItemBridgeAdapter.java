...
public class ItemBridgeAdapter extends RecyclerView.Adapter implements FacetProviderAdapter {
    ...
    public int getItemViewType(int position) {
        ...
        Presenter presenter = presenterSelector.getPresenter(item); // 根据选择器返回对应的 --> Presenter.
    ...
    public final RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        // 比如返回 ListRowPresenter. (VerticalGridView 中的 item样式).
        Presenter presenter = mPresenters.get(viewType);  // 返回 垂直 VerticalGridView 的 每个 item的Presenter.
        ...
        if (presenterView != null) {
            ...
            // 焦点事情.
            presenterView.setOnFocusChangeListener(viewHolder.mFocusChangeListener); // 设置item焦点监听.
        }
        if (mFocusHighlight != null) {
    ...
    public final void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        viewHolder.mItem = mAdapter.get(position); // 比如 demo 传入的 mAdapter -> ArrayObjectAdapter.
