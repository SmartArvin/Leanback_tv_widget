 ...
 *     Application should call {@link ViewHolder#setActivated(boolean)} to change
...
public abstract class RowPresenter extends Presenter {
     ...
     * application will do its own through {@link ViewHolder#setActivated(boolean)}.
     */
    ...
    static class ContainerViewHolder extends Presenter.ViewHolder {
        ...
        public ContainerViewHolder(RowContainerView containerView, ViewHolder rowViewHolder) {
            super(containerView);
            // 添加 rowViewHolder.view===> ListRowView(里面包含了HorizontalGridView).
            // 不一定是 ListRowView，可以是其它的.
    ...
    public static class ViewHolder extends Presenter.ViewHolder {
         ...
         * {@link ViewHolder#setActivated(boolean)}. No operation if
         * {@link ViewHolder#setActivated(boolean)} is never called.  Normally
    ...
    public final Presenter.ViewHolder onCreateViewHolder(ViewGroup parent) {
        ViewHolder vh = createRowViewHolder(parent); // 返回一个横向的 HorizontalGridView.
        vh.mInitialzed = false;
        Presenter.ViewHolder result;
        if (needsRowContainerView()) { // 判断是否要添加标题头. (后续加入隐藏标题头的函数).
    ...
    /**
     * Called after a {@link ViewHolder} is created for a Row.
    ...
    /**
     * Returns the {@link ViewHolder} from the given RowPresenter
     * ViewHolder.
     */
